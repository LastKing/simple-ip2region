 ...
 * 提供三种方式 ：
 * 官方是 是说 binary  btree  纯内存  三种方式，但是只有第前两种方式 node 客户端
 */
const fs = require('fs');

const ipbase = [16777216, 65536, 256, 1]; // for ip2long

const indexBlockLength = 12;
const totalHeaderLength = 4096;  //未知意义的固定值，其他的都是可赋值的。
...
/**
 * binary search async 异步版本，依赖于  koa的co执行，并不能单独执行。
 * @param ip
 * @returns {*}
 */
ip2region.binarySearch = function* (ip) {
  ...
  while (low <= high) {
    ...
    pos = firstIndexPtr + mid * indexBlockLength;
    // fs.readSync(this.db_fd, indexBuffer, 0, indexBlockLength, pos);
    yield read(this.db_fd, indexBuffer, 0, indexBlockLength, pos);

    // console.log(indexBuffer.toString('base64'));
    sip = getLong(indexBuffer, 0);
  ...
  yield read(this.db_fd, dataBuffer, 0, dataLen, dataPos);
 ...
 * btree  search async
 */
ip2region.btreeSearchAsync = function* (ip) {
  let indexBlockBuffer = new Buffer(indexBlockLength);
  let headerIndexBuffer = new Buffer(totalHeaderLength);
  ...
  // header index handler
  if (headerSip === null) {
    yield read(this.db_fd, headerIndexBuffer, 0, totalHeaderLength, 8);
  ...
  var blockLen = eptr - sptr;
  var blockBuffer = new Buffer(blockLen + indexBlockLength);
  yield read(this.db_fd, blockBuffer, 0, blockLen + indexBlockLength, sptr);
  ...
  yield read(this.db_fd, dataBuffer, 0, dataLen, dataPtr);
...
exports.create = function (db_path) {
  if (typeof db_path === "undefined" || !fs.existsSync(db_path)) {
    throw new Error("[ip2region] db file not exists : " + db_path);
  }
